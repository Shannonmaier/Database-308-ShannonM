...
/* 1. Display	the	cities	that	make	the	most	different	kinds	of	products.	Experiment	with	
the	rank() function */
...
SELECT 
    ...
    DENSE_RANK() OVER (ORDER BY COUNT(DISTINCT prodid) DESC) AS rank
FROM 
    Products
GROUP BY 
    city
...
/* Display the names of products whose priceUSD is less than 1% of	the average	 priceUSD, in	
...
/* 3. Display the customer	last name, product id	ordered, andthe totalUSD	for all	
orders	 made in March of any	year, sorted by	totalUSD from	low to	high. */
...
FROM 
    Orders
JOIN 
    People ON Orders.custId = People.pid
WHERE 
    EXTRACT(MONTH FROM dateOrdered) = 3
ORDER BY 
    Orders.totalUSD ASC;
...
/* 4. Display the last name	of all	customers (in reverse	alphabetical order) and their	
total	 ordered by customer,	and nothing more. Use coalesce to avoid showing NULL totals.*/

--ChatGPT CODE

SELECT 
    People.lastName, 
    COALESCE(SUM(Orders.totalUSD), 0) AS totalOrdered
FROM 
    People
INNER JOIN
	Customers on people.pid = customers.pid
LEFT JOIN 
    Orders ON People.pid = Orders.custId
GROUP BY 
    People.pid, People.lastName
ORDER BY 
...
/* 5. Display the names of all customers who bought products from agents based in Chilliwack 
along with	the names of	the products they ordered, and the names of	the	 
...
SELECT 
    ...
    agnt.lastName AS AgentLastName
FROM 
    Orders ord
JOIN 
    People cust ON ord.custId = cust.pid
JOIN 
    Products prod ON ord.prodId = prod.prodid
JOIN 
    People agnt ON ord.agentId = agnt.pid
WHERE 
    cust.pid IN (
        SELECT 
            distinct ord.custId
        ...
        JOIN 
            People agnt ON ord.agentId = agnt.pid
        WHERE 
            agnt.homeCity = 'Chilliwack'
    )
ORDER BY 
    cust.lastName, prod.name, agnt.lastName;
...
/* 6. Write a query	to check the accuracy	of the totalUSD column in the Orders 
table.	This means calculating Orders.totalUSD from data in other tables and comparing 
those values to the values in Orders.totalUSD. Display all rows in Orders where	 Orders.totalUSD
is incorrect, if any. If there are any incorrect values, explain why they are wrong. 
...
SELECT 
    o.orderNum,
    o.totalUSD AS provided_totalUSD,
    ROUND(SUM(p.priceUSD * o.quantityOrdered), 2) AS calculated_totalUSD
FROM 
    Orders o
JOIN 
    Products p ON o.prodId = p.prodid
GROUP BY 
    o.orderNum, o.totalUSD
HAVING 
    provided_totalUSD != calculated_totalUSD;


--CHANGED TO:
SELECT 
    orders.*,
    ROUND( ((products.priceUSD * orders.QuantityOrdered) * ((100-customers.discountPct)*.01)):: numeric, 2) AS calculated_totalUSD
FROM 
    Orders 
JOIN 
    Products ON orders.prodId = products.prodid
JOIN
	customers on orders.custid = customers.pid
JOIN
	 agents on orders.agentid = agents.pid

WHERE 
    orders.totalUSD != ROUND( ((products.priceUSD * orders.QuantityOrdered) * ((100-customers.discountPct)*.01)):: numeric, 2);
...
FROM 
    People AS c
INNER JOIN 
    People AS a ON c.pid = a.pid
WHERE 
    c.pid IN (SELECT custId FROM Orders)
    AND a.pid IN (SELECT agentId FROM Orders);
